...
router.post('/', requireAdmin, async (req, res) => {
	try {
    const { name, slug, domains, uberCustomerId, pickup, brandColor, locations, cities, hours, deliveryFeeCents, splitDeliveryFee, logoUrl, logoLinkUrl, stripeAccountId, deliveryProvider, doordashStoreId, tagline, stripePublishableKey, stripeSecretKey, uberClientId, uberClientSecret, uberEnv, doordashDeveloperId, doordashKeyId, doordashSigningSecret, uberWebhookSecret } = req.body || {};
		if (!name || !slug) return res.status(400).json({ error: 'name and slug are required' });
		const mock = req.app.locals.mockData;
    if (mock) {
      const created = { _id: `site-${Date.now()}`, name, slug, domains: domains || [], uberCustomerId, uberClientId, uberClientSecret, uberEnv, uberWebhookSecret, deliveryProvider: deliveryProvider === 'doordash' ? 'doordash' : 'uber', doordashStoreId, doordashDeveloperId, doordashKeyId, doordashSigningSecret, pickup, locations: Array.isArray(locations) ? locations : [], cities: Array.isArray(cities) ? cities : [], hours: hours || undefined, deliveryFeeCents: Number(deliveryFeeCents) || 0, splitDeliveryFee: !!splitDeliveryFee, brandColor: brandColor || '#0ea5e9', logoUrl, logoLinkUrl, tagline, isActive: true, stripeAccountId, stripePublishableKey, stripeSecretKey };
			mock.sites.unshift(created);
			try { saveMockData(req.app.locals.mockData); } catch {}
			return res.status(201).json(created);
		}
    const site = await Site.create({ name, slug, domains: domains || [], uberCustomerId, uberClientId, uberClientSecret, uberEnv, uberWebhookSecret, deliveryProvider: deliveryProvider === 'doordash' ? 'doordash' : 'uber', doordashStoreId, doordashDeveloperId, doordashKeyId, doordashSigningSecret, pickup, locations: Array.isArray(locations) ? locations : [], cities: Array.isArray(cities) ? cities : [], hours, deliveryFeeCents: Number(deliveryFeeCents) || 0, splitDeliveryFee: !!splitDeliveryFee, brandColor: brandColor || '#0ea5e9', logoUrl, logoLinkUrl, tagline, stripeAccountId, stripePublishableKey, stripeSecretKey, isActive: true });
